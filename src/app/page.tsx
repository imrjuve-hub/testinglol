          ...
          {participants.map(p => (
            <tr key={p.puuid} className="border-b border-neutral-900">
              <td className="td pr-2">
                <span className={`px-2 py-1 rounded-lg ${highlight(p, playerA, playerB) ? 'bg-blue-600/20 text-blue-200' : 'bg-neutral-800/60 text-neutral-200'}`}>{p.summonerName}</span>
              </td>
              <td className="td">
...
function highlight(p: any, a: string, b: string) {
  const name = (p.summonerName || '').toLowerCase();
  return !!a && !!b && (name === a.toLowerCase() || name === b.toLowerCase());
}
